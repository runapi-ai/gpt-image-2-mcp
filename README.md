...
RunAPI MCP server for the **GPT Image 2** model line. Create tasks,
poll their status, and check pricing through a single RunAPI API key.

## Tools

- `edit_image` — create a GPT Image 2 task (edit image) and (optionally) poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. Models: `gpt-image-2`.
- `text_to_image` — create a GPT Image 2 task (text to image) and (optionally) poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. Models: `gpt-image-2`.
...
```bash
npx -y @runapi.ai/gpt-image-2-mcp
```
