<h1 align="center">GPT Image 2 MCP Server</h1>

<p align="center">
  <strong>Create GPT Image 2 AI tasks through RunAPI — 2 endpoints, 1 model — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
`@runapi.ai/gpt-image-2-mcp` is a focused MCP server for the **GPT Image 2** model line on RunAPI.
It lets an MCP-compatible assistant create GPT Image 2 tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `edit_image` | Yes | Create a GPT Image 2 task (edit image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `text_to_image` | Yes | Create a GPT Image 2 task (text to image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a GPT Image 2 model and endpoint. |

---

## Models

GPT Image 2 covers 1 model across 2 endpoints. Each tool accepts the models listed for it:
